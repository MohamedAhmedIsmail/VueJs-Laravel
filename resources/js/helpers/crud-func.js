...
export const deleteItem = async function(item, button, deleteLink) {
    ...
    await that.axios.post( deleteLink + item.id ).then(function(){
        ...
        that.$iziToast.success({message: 'تم الحذف بنجاح', position: 'bottomCenter', timeout: 3000});
    })
    .catch(function(error){
      ...
      if (error.response) {
        if (error.response.status == 422) {
            ...
            that.$iziToast.error({message: 'لم يتم الحذف', position: 'bottomCenter', timeout: 3000});
        }
      }
    });
}
...
export const deleteFile = function(item, index, button) {
    ...
    that.axios.post( that.deleteLink, formData ).then(function(){
      button.disabled = false;
      that.fetchComponentData();
      that.$iziToast.success({message: 'تم الحذف بنجاح', position: 'bottomCenter'});
    })
    .catch(function(error){
      button.disabled = false;
      if (error.response) {
